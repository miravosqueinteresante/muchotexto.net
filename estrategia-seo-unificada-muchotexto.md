...
16. [Progreso completado](#16-progreso-completado)
...
**Total: 42 temas. 33 publicados (✅). 9 pendientes (7 ⬜ + 2 ⭐).**
...
- **Fuentes RSS:** 15 medios paraguayos
...
### 12.7 Pendientes conocidos (no críticos)
...
**Navegación:** Inicio | Observatorio IA | Glosario | Categorías | Acerca de | Cómo trabajamos | Contacto
...
├── _data/navigation.yml     # Menú de navegación (7 items)
├── scripts/
...
| 3 | Publicar artículo #5: Hidrógeno verde (Pilar 1) | Contenido |
| 4 | Publicar artículo #7: Energía renovable y cambio climático (Pilar 1) | Contenido |
| 5 | Publicar artículo #34: E-commerce y logística (Pilar 4) | Contenido |
...
| # | Acción | Área |
|---|---|---|
| 8 | Crear/optimizar perfil de LinkedIn con keywords del nicho | Autoridad |
| 9 | Preparar "media kit" de una página | Autoridad |
| 10 | Identificar 3-5 periodistas de tecnología en medios paraguayos | Autoridad |
| 11 | Crear cuenta/blog en LinkedIn para publicar versiones resumidas | Distribución |
| 12 | Preparar post de LinkedIn anunciando el cluster IA+Paraguay | Distribución |
| 13 | Crear project board (GitHub Projects) con los 11 temas pendientes | Planificación |
...
**Schema y E-E-A-T:**
- `{% seo %}` reemplazado por `_includes/seo-meta.html`. Elimina el WebSite duplicado sin `@id` que generaba jekyll-seo-tag y la Organization con nombre "César Sánchez" hardcodeado por el plugin. Solo quedan nuestros 3 bloques con `@id`: Person, Organization, WebSite (con SearchAction). Meta tags OG/Twitter/canonical preservados.
- `{% feed_meta %}` eliminado (generaba link rss+xml duplicado). Solo queda atom+xml en seo-meta.
- `og:image:width/height/type` duplicados eliminados de custom-head (ya están en seo-meta).
- CollectionPage agregado a homepage y `/ia-en-paraguay/`. Conteo dinámico vía Liquid.
- `social.name: MuchoTexto` en `_config.yml` — paso intermedio antes del reemplazo completo.

**Arquitectura semántica hub-and-spoke:**
- Todas las páginas del observatorio incluyen `isPartOf: /ia-en-paraguay/#collection` en su TechArticle.
- `/glosario/` → DefinedTermSet con 5 DefinedTerms representativos.
- `/directorio/` → ItemList con 5 ListItem→Organization.
- Google entiende el pillar page como centro de autoridad y las páginas periféricas como dependencias.

**Homepage — "Sobre el observatorio":**
- Misión: "El observatorio que investiga cómo la IA redefine la energía, la economía y la sociedad paraguaya."
- Texto contextual con link directo al observatorio.
- Micro-CTA "Conocé todas las secciones:".
- 6 tarjetas visuales con íconos diferenciados: Cronología, Mapa regulatorio, Directorio, Casos de uso, Glosario vivo, Así trabajamos.
- Foto de César Sánchez en author bio de post + page layouts (reemplaza iniciales "CS").

**Schema y E-E-A-T:**
- CollectionPage agregado a homepage y `/ia-en-paraguay/` — señal de hub temático para Google. `mainEntity` apunta al observatorio.
- WebSite duplicado eliminado de index.markdown. Solo queda el de jekyll-seo-tag.
- Person mejorado: `image` (foto WebP 200x200, 5.9KB), `description` (bio profesional), `knowsAbout` granular (Data Centers, Geopolítica Tecnológica, Regulación Digital, Automatización — alineado a los 5 pilares).
- Organization: `publishingPrinciples` + `correctionsPolicy` → `/como-trabajamos/`. Señal E-E-A-T de transparencia editorial.
- Foto de César Sánchez en author bio de post + page layouts (reemplaza iniciales "CS").
- BreadcrumbList en homepage evaluado y descartado (1 solo ítem no tiene valor en Google).

**Homepage — rediseño completo:**
- Hero con conteo dinámico de artículos, 5 pilares como navegación secundaria, CTA al observatorio
- 3 tarjetas de últimos análisis con sector, título, descripción completa y fecha
- 2 tarjetas lado a lado: Pulso Paraguay + Editorial más recientes
- Sección "Sobre el observatorio" con descripción extendida y links a páginas vivas
- Tags técnicos reemplazados por nombres de sector (Infraestructura y energía, etc.)
- Conteo dinámico vía Liquid: `\` | minus: 2\`` (excluye artículos no-IA). Auto-update en cada build.
- Descripciones completas sin truncado ni `...`
- Paginación deshabilitada. Homepage convertida en landing page.
- CSS: nuevo partial `_home.scss` (~200 líneas). CSS total compilado: ~24KB (4.6KB crítico inline).
- Gap masthead-hero reducido: 4rem → 1rem.

**Observatorio:**
- Pillar page: conteo dinámico vía Liquid (misma lógica que homepage)
- Artículo #38 (Fútbol) removido del observatorio — no es análisis de IA. Conteo: 31.
- Artículo #19 (Bienvenidos) excluido del conteo del observatorio.
- Description del pillar page acortada: 196 → 149 chars.

**Infraestructura:**
- `fix_pulso_descriptions.py`: corregido `basename` → `os.path.basename` (bloqueaba el workflow editorial en CI)
- Workflow editorial re-triggerado exitosamente. Editorial del 18-jul generada.

**Documentación:**
- §2.1: conteos actualizados (33 artículos, 33+ editoriales, 54+ pulsos, 120+ total)
- §3.2: nota de desglose actualizada (31 observatorio / 33 total)
- §7.3: CSS corregido, paginación deshabilitada documentada
- §16: 7 entradas nuevas de progreso
- §18: esta entrada

### 20 de julio 2026

**Descripciones y títulos automatizados:**
- `fix_editorial_descriptions.py`: si la primera oración excede 160 chars, se toma completa sin truncar.
- `fix_pulso_descriptions.py`: eliminado truncado con `...`. Formato `Tema: oración. Pulso Paraguay — fecha.` completo.
- `pulso_diario.py`: eliminado `…` del truncado de títulos. Truncado limpio sin marcador.
- 24 editoriales + 56 pulsos regenerados con las nuevas reglas.

**Homepage y directorio:**
- Ícono "Casos de uso": ⚋ → ◊ (se renderizaba como emoji a color en iOS).
- Directorio: link Smart City Paraguay → instagram.com/smartcityparaguay.

**Métricas:**
- Core Web Vitals baseline registrados: 91 Performance, LCP 3.3s, CLS 0, TBT 150ms. Documentado en §2.4.
- GSC baseline registrado: 8 clics, 160 impresiones, CTR 5%, posición 5.0. Consulta principal: `muchotexto` (34 imp, 0 clics). Documentado en §2.3.
- Contraste corregido: opacity .5→.7 en tagline, analysis-meta, observatorio-desc.

### 18 de julio — revisión 2

**Documentación y calidad:**
- §1 reestructurado: renombrado a "Producto y visión" con §1.1 "Qué es Muchotexto" (formatos, audiencias, exclusiones) y §1.2 "Visión y posicionamiento"
- §3.2: 7 artículos corregidos (⬜→✅), conteo actualizado a 32/10. Nota agregada sobre artículo extra "qué es un data center" fuera del plan de 42 temas.
- §7.1: SpeakableSpecification corregido (custom-head→post.html). WebSite duplicado en homepage documentado como intencional.
- §7.3: Anthropic-control eliminado de robots.txt. Referencia a "8 bots" reemplazada por lista explícita de 6.
- §13.2: decisión de diseño documentada — páginas del observatorio fuera de nav principal.
- §13.3: mapa de archivos actualizado (10→16 scripts, 6 archivos nuevos agregados).
- §14.1: paso 10 agregado al flujo de publicación — actualizar §3.2 de este documento.
- §15: prioridades actualizadas. Artículos ya publicados (#23,#24,#31) removidos. Nuevos pendientes: #5,#7,#34.
- §16: tabla de progreso actualizada con 7 nuevos artículos + renumeración completa (63 items).
- §2.3: sección nueva "Resultados medidos" (placeholder). Reconoce explícitamente el gap de medición.
- Pillar page: Biblioteca rediseñada con listas verticales + descripciones. Artículo #38 agregado. Conteo corregido 31→32. Description acortada 196→149 chars. last_modified_at actualizado.

**Infraestructura:**
- robots.txt: Anthropic-control eliminado (no es un user-agent real de Anthropic). Desplegado.
- Pillar page description: acortada de 196 a 149 chars. Desplegado.

### 18 de julio 2026

**Schema y grafo de entidades:**
- Schema Article JSON-LD en `_layouts/post.html` — cierra el grafo Person ↔ Organization ↔ Article ↔ FAQPage por @id
- Schema TechArticle JSON-LD en `_layouts/page.html` — cubre pillar page, glosario, directorio, cronologia, regulacion, casos-de-uso, como-trabajamos
- sameAs unificado: Person + Organization → `https://x.com/cesanz`
- TechArticle inline duplicado removido de la página pilar

**IndexNow:**
- `scripts/ping_indexnow.py` — lee sitemap, dedupea contra cache, notifica URLs nuevas
- Paso en `jekyll.yml` tras build. Removido código muerto de `validate_publish.py`

**Descripciones automatizadas:**
- Pulso Paraguay: fix de 32 posts, auto-fix en `pulso-diario.yml`
- Editorial: fix de 22 posts, auto-fix en `editorial-diario.yml`
- Editorial titles: coma entre cláusulas (system prompt + 3 títulos corregidos)

**Bing Webmaster Tools:**
- Meta description de `/ia-en-paraguay/` acortada: 196 → 149 chars

**Documentación:**
- Estrategia SEO unificada reestructurada con TOC, secciones numeradas, UTF-8 limpio
- Contexto operativo fusionado en este documento. `muchotexto_contexto_proyecto.md` eliminado.

### 9 de julio 2026

**Artículos publicados:**
- Artículo #10: Paraguay entre China y Taiwán (Pilar 2)
- Artículo #18: Educación tech en Paraguay (Pilar 3)

**Mejoras técnicas:**
- FAQ schema obligatorio: 15 artículos recibieron FAQPage (45 preguntas totales)
- Author bio en `_layouts/post.html`: avatar, bio, links a /about/, Twitter y LinkedIn
- Related posts por pilar: prioriza artículos del mismo tag paraguas
- LinkedIn en footer
- Editorial titles fix: prompt pide ≤45 chars para GPT

**Ortografía y encoding:**
- Regla: título y description con acentos correctos. Script `fix_titles.py` creado.
- Página pilar reescrita con UTF-8 limpio

**Infraestructura:**
- Pre-commit hook: `validate_publish.py` con 12 checks
- Sitemap: homepage incluida
- Descripción única en paginación
