...
12. **Actualización del observatorio y la estrategia**: antes del commit final, si el artículo aporta nuevos hitos, entidades, casos o términos, actualizar las páginas del observatorio que correspondan:
    ...
    Las páginas de entidades (`/entidades/`) y el grafo (`/grafo/`) no se editan a mano: se regeneran con `python scripts/build_entities.py` tras publicar artículos nuevos. El dashboard (`/dashboard-energetico/`) se actualiza por cadencia de re-verificación de datos (cada 30 días), no por artículo; sus series de generación y suministro se sincronizan solas desde la capa de datos en cada build (ver "Capa de datos verificables" más abajo), y solo el canon editorial (tarifas, proyecciones, timeline) se edita en `_data/energia.yml`.
