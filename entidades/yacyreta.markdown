---
layout: entidad
title: "Entidad Binacional Yacyretá"
description: >
  Perfil de Yacyretá en el Observatorio de IA en Paraguay: articulos, leyes y fuentes verificables sobre Entidad Binacional Yacyretá.
permalink: /entidades/yacyreta/
last_modified_at: 2026-09-25
entity_id: yacyreta
entity_name: Yacyretá
entity_name_full: Entidad Binacional Yacyretá
entity_description: >
  La Entidad Binacional Yacyretá (EBY), creada con Argentina en 1973, es la segunda mayor hidroeléctrica de Paraguay. Aporta alrededor de 1.550 MW al sistema paraguayo (18% de la capacidad instalada) y cerca del 11% de la generación de la matriz. Junto con Itaipú concentra el 97% de la capacidad hidroeléctrica del país en la cuenca del Paraná. Su serie de generación histórica se publica en MuchoTexto Data.
entity_category: infraestructura
entity_datos_url: https://datospublicos.muchotexto.net/yacyreta.html
entity_datos_desc: "Generación binacional Paraguay-Argentina: mensual EBY y agregada anual"
related_articles:
  - title: "Yguazú Digital y la apuesta de Paraguay por convertirse en hub de IA"
    url: /articulos/2026/06/23/yguazu-digital-paraguay-hub-ia-mas-grande-del-mundo/
  - title: "Paraguay tiene energía para la IA pero no la red para transportarla"
    url: /articulos/2026/07/08/red-electrica-paraguay-ia/
  - title: "Paraguay tiene energía 100% renovable y el cambio climático la amenaza"
    url: /articulos/2026/08/03/energia-renovable-cambio-climatico-paraguay/
entity_laws:
  - "Tratado de Yacyretá (1973) — entidad binacional Paraguay-Argentina"
obs_casos-de-uso:
  - label: "Diagnóstico de la tarifa eléctrica"
    url: /articulos/2026/08/20/estudio-ceare-tarifa-paraguay/
    context: "El estudio CEARE-UBA (con apoyo del BID, diciembre 2025) proyecta la tarifa media de la ANDE de 49,2 a 68,6 US$/MWh entre 2026 y 2030 (+39,4%). El costo medio de generación subió de 24 a ~28 US$/MWh..."
  - label: "Modernización institucional del sector eléctrico"
    url: /articulos/2026/08/19/modelo-institucional-psr-sector-electrico/
    context: "El documento de la consultora PSR (septiembre 2025, para el MIC/PNUD) diagnostica funciones mezcladas entre VMME y ANDE y propone separarlas en ministerio, regulador y empresa. Coincide con Uruguay,..."
---

3 articulos en el observatorio mencionan a Yacyretá.
